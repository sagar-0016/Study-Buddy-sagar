...
import { gocFlashcards } from '@/lib/flashcard-data';
...
export default function GocDeck() {
  ...
  const progress = gocFlashcards.length > 0 ? ((gocFlashcards.length - availableCards.length) / gocFlashcards.length) * 100 : 0;
    ...
    <div className="flex flex-col h-full min-h-[75vh]">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
        <div className="text-center sm:text-left">
          <h1 className="text-2xl font-bold tracking-tight">General Organic Chemistry</h1>
          <p className="text-muted-foreground">Master the fundamental concepts of GOC.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" asChild>
            <Link href="/flashcards/chemistry">
      ...
      <div className="mb-6 space-y-2">
        ...
        <p className="text-sm text-muted-foreground text-center">
            {gocFlashcards.length - availableCards.length} / {gocFlashcards.length} cards completed
        </p>
      </div>

...
import { forcesFlashcards } from '@/lib/flashcard-data';
...
export default function ForcesDeck() {
  const [isFlipped, setIsFlipped] = useState(false);
  const [cardStatuses, setCardStatuses] = useLocalStorage<Record<string, CardStatus>>('forces-statuses', {});
  const [shuffledCards, setShuffledCards] = useLocalStorage('forces-shuffled-deck', []);
    ...
    <div className="flex flex-col h-full min-h-[75vh]">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
        <div className="text-center sm:text-left">
          <h1 className="text-2xl font-bold tracking-tight">Forces & Newton's Laws</h1>
          <p className="text-muted-foreground">Master the principles of force and motion.</p>
        </div>
      ...
      <div className="mb-6 space-y-2">
        ...
        <p className="text-sm text-muted-foreground text-center">
            {forcesFlashcards.length - availableCards.length} / {forcesFlashcards.length} cards completed
        </p>
      </div>
